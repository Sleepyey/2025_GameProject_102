...
public class FruitGame : MonoBehaviour
{
    ...
    public float fruitStartHeight = 5f;                 //과일 시작 높이 (인스펙터에서 조절 가능)
    ...
    void Update()
    {
        ...
        {
            Vector3 mousePosition = Input.mousePosition;                            //마우스 위치에 따라 X 좌표만 이동 시키기 위해 사용
            Vector3 worldPosition = mainCamera.ScreenToWorldPoint(mousePosition);

            Vector3 newPosition = currentFruit.transform.position;                  //과일 위치 업데이트 (X좌표만, Y는 그대로 유지)
            newPosition.x = worldPosition.x;

            float halFruitSize = fruitSizes[currentFruitType] / 2;                  //과일이 원이기 때문에 반 값을 나워서 바구니 밖으로 안나가게 설정
    ...
    public void MergeFruits(int fruitType, Vector3 position)
    {
        ...
        {
            GameObject newFruit = Instantiate(fruitPrefabs[fruitType + 1], position, Quaternion.identity);      //다음 단걔 과일 생성
    ...
    void SpawnNewFruit()            //과일 생성 함수
    {
        if (!isGameOver)                        //게임 오버가 아닐 때만 새 과일 생성
        {
            ...
            Vector3 spawnPosition = new Vector3(worldPosition.x, fruitStartHeight, 0);      //X 좌표만 사용하고 Y는 설정된 높이로, Z는 2D라서 0으로 설정
