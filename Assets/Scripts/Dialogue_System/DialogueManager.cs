...
public class DialogueManager : MonoBehaviour
{
    ...
    public bool skipTypibgOnClick = true;                   //클릭 시 타이핑 즉시 완료 할지 여부

    //내부 변수들
    private DialogueDataSO currentDialogue;                     //현재 진행 중인 대화 데이터
    public int currentLineIndex = 0;                           //현재 몇 번째 대화 중인지 (0부터 시작)
    ...
    public void StartDialogue(DialogueDataSO dialogue)          //새로운 대화를 시작 하는 함수
    {
        ...
        if (characterImage != null)
        {
            ...
            {
                characterImage.sprite = defaultCharacterImage;          //기본 데이터의 이미지 사용
            }
        }
    ...
    IEnumerator TypeText(string textToText)         //타이핑 할 전체 텍스트
    {
        ...
        {
            dialogueText.text += textToText[i];             //한 글자 추가
    ...
    {
        if (currentDialogue != null && currentLineIndex < currentDialogue.dialogueLines.Count)  //대화 데이터와 인덱스가 유요한지 확인
        {
    ...
    public void ShowNextLine()      //다음 대화 줄로 이동 시키는 함수 (타이핑이 완료된 후에만 호출)
    {
        ...
        {
            ShowCurrentLine();      //ㅐ화가 남았으면 다음 줄 표시
        }
    }
